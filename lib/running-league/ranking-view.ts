...
export function getRankingViewDescription(view: RankingView, periodLabel = '현재 기간'): string {
  if (view === 'pb') {
    return '거리별 PB · 5km / 10km / Half / Full · 기록이 짧을수록 상위 (초 단위 오름차순)'
  }
  if (view === 'beat_rival') {
    return '이겨라 대상 회원을 넘어서는 순위 경쟁 · 기간 마일리지 기준'
  }
  if (view === 'attendance') {
    return `${periodLabel} · ${ATTENDANCE_KING_DAY_RULE_LABEL} · 출석 일수 많을수록 상위`
  }
  return `${periodLabel} 누적 거리 · 많을수록 상위 (내림차순)`
}

export const MILEAGE_RANKING_SORT_HINT = '기간 누적 거리 기준 · 내림차순 정렬'
