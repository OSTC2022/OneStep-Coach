...
COMMENT ON COLUMN public.profiles.approval_status IS
  'pending: 승인 대기, approved: 접속 허용, rejected: 거절, on_hold: 보류(일시 정지)';

-- 신규 가입 트리거: on_hold 허용 (메타데이터 방어)
...
BEGIN
  ...
  IF v_profile_role = 'admin' THEN
    v_approval := 'approved';
  ELSIF v_approval NOT IN ('pending', 'approved', 'rejected', 'on_hold') THEN
    v_approval := 'pending';
  END IF;
